...
const Bookings = () => {
  ...
  useEffect(() => {
    Promise.all([bookingService.getAll(), packageService.getAll()])
      .then(([bRes, pRes]) => {
      ...
      <div className="container">
        ...
        {loading && (
          <div className="page-loader" style={{ minHeight: 300 }}>
            <span className="spinner spinner-dark" /> Loading bookings...
          </div>
        )}
        {error && (
                    ...
                    <div className={styles.cardTop}>
                      <div>
                        <h3 className={styles.cardTitle}>
                          {b.serviceName || b.packageName || "Tour Package"}
                        </h3>
                        {pkg.destination && (
                          <p className={styles.cardDest}>
                            <i className="fas fa-map-marker-alt" />{" "}
                            {pkg.destination}
                          </p>
                        )}
                        {b.bookingId && (
                          <p className={styles.bookingId}># {b.bookingId}</p>
                        )}
                      </div>
                      <div className={styles.priceTag}>
                        {b.totalPrice || b.price ? (
                          <>
                            <span className={styles.priceFrom}>Total</span>
                            <span className={styles.price}>
                              $
                              {Number(b.totalPrice || b.price).toLocaleString()}
                            </span>
                          </>
                        ) : null}
                      </div>
                      ...
                      <div className={styles.metaItem}>
                        ...
                        <div>
                          ...
                          <span className={styles.metaValue}>
                            {fmt(b.bookingDate || b.travelDate)}
                          </span>
                        </div>
                      </div>
                      {b.bookingTime && (
                      ...
                      <div className={styles.metaItem}>
                        ...
                        <div>
                          ...
                          <span className={styles.metaValue}>
                            ...
                            {(b.numberOfPeople || b.numberOfGuests || 1) === 1
                              ? "person"
                              : "people"}
                          </span>
                        </div>
