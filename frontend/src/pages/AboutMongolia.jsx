...
const FALLBACK = [
  ...
  },
  {
    ...
    category: "food",
    description:
      "Mongolian cuisine revolves around meat and dairy — try tsuivan (stir-fried noodles), khorkhog (stone barbecue), and airag (fermented mare's milk).",
    readMore:
      "Mongolian food reflects the nomadic lifestyle — hearty, energy-rich, and based on what the land provides. Buuz are steamed mutton dumplings eaten during Tsagaan Sar. Aaruul (dried curds) are a popular snack. Khorkhog, meat cooked with hot stones inside a sealed container, is a celebratory dish. Airag (fermented mare's milk) is mildly alcoholic and offered to guests as a sign of hospitality.",
    images: [
  ...
  },
  {
    ...
    category: "culture",
    description:
      "Khoomei (throat singing) is a unique vocal art where singers produce multiple pitches simultaneously — a UNESCO Intangible Cultural Heritage.",
    readMore:
      "Khoomei has been practiced for centuries in western Mongolia and the Tuvan regions. Singers manipulate their vocal tract to produce a drone and a melodic whistle simultaneously, mimicking the sounds of nature. Different styles include sygyt (high whistle), kargyraa (deep rumble), and khoomei (mid-range). It is performed at festivals, ceremonies, and for spiritual purposes.",
    images: [
  ...
  },
  {
    ...
    category: "nature",
    description:
      'Known as the "Blue Pearl of Mongolia," Khövsgöl holds nearly 70% of Mongolia\'s freshwater and is surrounded by pristine taiga forests.',
    readMore:
      "Khövsgöl Lake is 136 km long, up to 262 m deep, and one of the 17 ancient lakes in the world. It is the world's 14th largest freshwater lake by volume. The lake remains frozen from January to May. The Tsaatan (reindeer herders) live in the surrounding forests. Activities include horse trekking, fishing, kayaking in summer, and dog sledding and ice fishing in winter.",
    images: [
      "https://images.unsplash.com/photo-1531804055935-76f44d7caff8?w=600&q=80",
    ],
  },
];
    ...
    <div className={styles.rmBackdrop} onClick={handleBackdrop}>
      <div className={styles.rmContent}>
        ...
        <div className={styles.rmImgWrap}>
          ...
          {item.category && (
            <span
              className={`${styles.catBadge} ${styles["cat_" + item.category] || ""}`}
            >
              <i className={CAT_ICONS[item.category] || "fas fa-compass"} />{" "}
              {item.category}
            </span>
          )}
        </div>
...
export default function AboutMongolia() {
  ...
  const tr = (item, key) =>
    item.translations?.[language]?.[key] || item[key] || "";
        ...
        >
          ...
          <div className={styles.catTabs}>
            ...
            {CATEGORIES.map((c) => (
              ...
              >
                <i className={CAT_ICONS[c] || "fas fa-compass"} />{" "}
          ...
          <div className={styles.grid}>
            {items.map((item) => {
              ...
              return (
                <div
                  key={item._id}
                  className={`${styles.card} ${styles["cat_" + item.category]}`}
                >
                    ...
                    <div className={styles.cardImgOverlay}>
                      {item.category && (
                        <span className={styles.catBadge}>
                          <i
                            className={
                              CAT_ICONS[item.category] || "fas fa-compass"
                            }
                          />{" "}
                          {item.category}
                        </span>
                      )}
                      <h3 className={styles.cardImgTitle}>
                        {tr(item, "title")}
                      </h3>
                    </div>
                    {imgs.length > 1 && (
                      <span className={styles.imgCount}>
                        <i className="fas fa-images" /> {imgs.length}
                      </span>
                    )}
                  </div>
                  <div className={styles.cardBody}>
                    <p className={styles.cardDesc}>
                      {tr(item, "description")?.slice(0, 120)}
                      {(tr(item, "description")?.length || 0) > 120 ? "…" : ""}
                    </p>
                    <button
