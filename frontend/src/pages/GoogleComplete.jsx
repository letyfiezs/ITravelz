...
export default function GoogleComplete() {
  ...
  useEffect(() => {
    ...
    if (!t || !raw) { navigate('/login?error=google_failed', { replace: true }); return; }
    try {
      setToken(t);
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await api.put('/auth/google/complete', { phone }, {
    ...
    <div className={styles.authPage}>
      ...
      <div className={styles.authLeft}>
        <div className={styles.authLeftContent}>
          ...
          <h2>Almost there!</h2>
          <p>Your Google account has been connected. Just add your phone number to complete your ITravelz profile.</p>
      ...
      <div className={styles.authRight}>
        <div className={styles.authCard}>
          <div className={styles.authHeader}>
            ...
            <p>Welcome, <strong>{userData.name}</strong>! Add your phone number to complete registration.</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className={styles.authForm}>
            <div className="form-group">
              ...
              <div className={styles.inputWrap}>
                ...
                <input className="form-input" type="text" value={userData.name} disabled style={{ opacity: 0.7 }} />
              </div>
            </div>
            <div className="form-group">
              ...
              <div className={styles.inputWrap}>
                ...
                <input className="form-input" type="email" value={userData.email} disabled style={{ opacity: 0.7 }} />
              </div>
            </div>
            <div className="form-group">
            ...
            <button type="submit" className={`btn btn-primary ${styles.submitBtn}`} disabled={loading}>
              {loading ? <><span className="spinner" /> Saving...</> : <>Complete Registration <i className="fas fa-arrow-right" /></>}
            </button>
          </form>

          <p className={styles.switchText}>
            <button onClick={handleSkip} style={{ background: 'none', border: 'none', color: 'var(--primary)', cursor: 'pointer', fontSize: 'inherit', textDecoration: 'underline' }}>
              Skip for now
            </button>
          </p>
        </div>
